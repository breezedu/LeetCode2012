...
public class ThreeSum {
	...
	//	int sum = 0;
		checkAllThreeSets(array, 0, 0, threeSet, retAL);
		
		return retAL;
	}

	private static void checkAllThreeSets(int[] array, int start, int sum, ArrayList<Integer> threeSet, ArrayList<ArrayList<Integer>> retAL) {
		...
		} else if(threeSet.size() <3) {
			
			for(int i= start; i<array.length; i++){
				ArrayList<Integer> newThreeSet = new ArrayList<Integer>(threeSet);
				newThreeSet.add(array[i]);
				int newSum = sum + array[i];
				checkAllThreeSets(array, i+1, newSum, newThreeSet, retAL);
